import { Component, inject } from '@angular/core';
import { CommonModule } from '@angular/common';
import { AdminStatistics } from '../admin-statistics/admin-statistics';
import { AdminSessions } from '../admin-sessions/admin-sessions';
import { AdminSubmissions } from '../admin-submissions/admin-submissions';
import { AdminBugReports } from '../admin-bug-reports/admin-bug-reports';
import { AdminCourses } from '../admin-courses/admin-courses';
import { SupabaseService } from '../../core/services/supabase';
import { TranslationService } from '../../core/services/translation.service';
import { Router } from '@angular/router';
import { BugReport } from '../../components/bug-report/bug-report';

@Component({
  selector: 'app-admin-layout',
  standalone: true,
  imports: [CommonModule, AdminStatistics, AdminSessions, AdminSubmissions, AdminBugReports, AdminCourses, BugReport],
  templateUrl: './admin-layout.html'
})
export class AdminLayout {
  private readonly router = inject(Router);
  private readonly supabaseService = inject(SupabaseService);
  readonly t = inject(TranslationService);

  activeTab: 'dashboard' | 'courses' | 'sessions' | 'submissions' | 'bugs' = 'dashboard';
  sidebarOpen = false;

  toggleSidebar(): void {
    this.sidebarOpen = !this.sidebarOpen;
  }

  onNavigate(tab: string): void {
    this.activeTab = tab as 'dashboard' | 'courses' | 'sessions' | 'submissions' | 'bugs';
    // Close sidebar on mobile when navigating
    this.sidebarOpen = false;
  }

  async logout(): Promise<void> {
    await this.supabaseService.signOut();
    this.router.navigate(['/login']);
  }
}
